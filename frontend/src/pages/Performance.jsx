                    ...
                    {isHROrAdmin && companyPerformance && (
                        <>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {companyPerformance.top_performers.slice(0, 6).map((tp, i) => (
                                            <div key={i} className="flex items-center gap-4 p-4 rounded-2xl bg-white/50 border border-slate-100 hover:bg-white transition-all group">
                                                <div className="w-10 h-10 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-black group-hover:scale-110 transition-transform">
                                                    #{i + 1}
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <div className="font-black text-slate-900 truncate">{tp.employee_name}</div>
                                                ...
                                                <div className="text-right">
                                                    <div className="flex items-center gap-1">
                                                        ...
                                                        <span className="font-black text-slate-900">{tp.avg_rating}</span>
                                                    </div>
                            ...
                            {companyPerformance.department_performance?.length > 0 && (
                                <div className="glass-morphism rounded-[2.5rem] p-8 border border-white/50">
                                    <h3 className="text-lg font-black text-slate-900 mb-6">Department Performance</h3>
                                        ...
                                        {companyPerformance.department_performance.map((dp, i) => (
                                            <div key={i} className="space-y-2">
                                                <div className="flex justify-between items-end">
                                                    ...
                                                    <span className="text-sm font-black text-indigo-600">{dp.avg_rating}/5 ({dp.employee_count} employees)</span>
                                                </div>
                    ...
                    {canReview && (
                        <div className="glass-morphism rounded-[2.5rem] p-8 border border-white/50">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                {!showReviewForm && (
                                    <button onClick={() => setShowReviewForm(true)} className="btn btn-primary px-6 py-2 text-sm font-bold shadow-lg shadow-indigo-600/20">
                            ...
                            {showReviewForm && (
                                <form onSubmit={handleSubmitReview} className="space-y-5 max-w-xl">
                                    <div>
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block">Employee</label>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <span className="ml-3 self-center text-lg font-black text-slate-700">{reviewForm.rating}/5</span>
                                        </div>
                                    </div>

                                    <div>
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block">Strengths</label>
                                    ...
                                    <div>
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block">Areas for Improvement</label>
                                    ...
                                    <div>
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block">Additional Comments</label>
                                    ...
                                    <div className="flex gap-3">
                                        <button type="submit" disabled={submitting} className="btn btn-primary px-8 py-3 font-bold shadow-lg shadow-indigo-600/20">
                        ...
                        <div className="flex justify-between items-center mb-6">
                            ...
                            {myPerformance.avg_rating > 0 && (
                                <div className="flex items-center gap-2 px-4 py-2 bg-indigo-50 rounded-xl">
                                    ...
                                    <span className="font-black text-indigo-600">{myPerformance.avg_rating}/5 avg</span>
                                </div>
                            )}
                        </div>
...
const ReviewCard = ({ review, showEmployee = false }) => (
    <div className="p-5 rounded-2xl bg-white/50 border border-slate-100 hover:bg-white transition-all">
        <div className="flex justify-between items-start mb-3">
            <div>
                {showEmployee && <div className="font-black text-slate-900">{review.employee_name}</div>}
        ...
        {review.strengths && (
            <div className="mb-2">
                <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Strengths: </span>
                <span className="text-sm text-slate-600">{review.strengths}</span>
            </div>
        )}
        {review.improvements && (
            <div className="mb-2">
                <span className="text-[10px] font-black text-amber-600 uppercase tracking-widest">Improvements: </span>
