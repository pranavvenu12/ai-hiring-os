                        ...
                        <div className="flex flex-col md:flex-row items-center gap-8">
                            <div className="flex gap-4">
                                <button
                                    ...
                                    className="btn btn-primary px-8 py-4 text-base font-black rounded-2xl shadow-xl shadow-indigo-600/20 disabled:opacity-40 disabled:cursor-not-allowed"
                                >
                                ...
                                <button
                                    ...
                                    className="btn bg-rose-50 text-rose-600 border border-rose-100 hover:bg-rose-500 hover:text-white px-8 py-4 text-base font-black rounded-2xl transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                                >
                    ...
                    {isHROrAdmin && companyAttendance && (
                        <div className="space-y-6">
                            ...
                            <div className="glass-morphism rounded-[2.5rem] p-8 border border-white/50">
                                <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-6">Attendance Distribution</h4>
                                <div className="flex items-center gap-8">
                                    <div className="w-40 h-40 relative">
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center">
                                            <div className="text-center">
                                                <div className="text-2xl font-black text-slate-900">{companyAttendance.present_percentage}%</div>
                                    ...
                                    <div className="flex-1 space-y-4">
                                        ...
                                        <div className="pt-2 border-t border-slate-100">
                                            <div className="text-sm font-black text-slate-700">Avg Working Hours: <span className="text-indigo-600">{companyAttendance.avg_hours}h</span></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <table className="w-full text-left">
                                    <thead>
                                        <tr className="bg-slate-50/50">
                                            ...
                                            <th className="px-6 py-4 text-xs font-black text-slate-400 uppercase tracking-widest">Clock In</th>
                                            <th className="px-6 py-4 text-xs font-black text-slate-400 uppercase tracking-widest">Clock Out</th>
                                            <th className="px-6 py-4 text-xs font-black text-slate-400 uppercase tracking-widest">Hours</th>
                            ...
                            {myAttendance.records.map((r, i) => (
                                <div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-white/50 border border-slate-100 hover:bg-white transition-all">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="font-bold text-slate-900">{new Date(r.attendance_date).toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}</div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        <span className="text-sm font-black text-indigo-600">{r.total_hours ? `${r.total_hours}h` : '—'}</span>
                                        <StatusBadge status={r.status} />
                                    </div>
                                </div>
                            ))}
...
const MiniStat = ({ label, value }) => (
    <div className="p-3 rounded-xl bg-white/50 border border-slate-100 text-center">
        ...
        <div className="text-sm font-black text-slate-700 capitalize">{value}</div>
    </div>
);
...
const StatCard = ({ icon: Icon, label, value, color }) => (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        className="glass-morphism p-6 rounded-[2rem] border border-white/50 hover:shadow-2xl transition-all duration-500 group">
...
const StatusBadge = ({ status }) => {
    ...
    return (
        <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${styles[status] || styles.not_clocked_in}`}>
